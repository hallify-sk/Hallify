<script lang="ts">
  ...
  <div class="fixed right-0 top-0 w-60 bg-background-100 h-screen pt-12 px-1 flex flex-col overflow-y-auto gap-1">
	{#if tables.length}
	  {#each tables as table}
		<button
		  class="bg-background-50 rounded-md block group relative"
		  on:click={() => {
			if ($tableList.filter(i => i.name.includes(table.stageReferenceName)).length < table.count) {
			  addTable(
		...
		>
		  <div class="w-full h-full rounded-md bg-black/80 group-hover:grid hidden absolute place-items-center">
		  ...
		  <div class="p-2">
			<div class="flex flex-row content-between items-center">
			  ...
			  <p class="float-right ml-auto {$tableList.filter(i => i.name.includes(table.stageReferenceName)).length == table.count ? 'text-red-500' : 'text-text-400'}">
				{table.count - $tableList.filter(i => i.name.includes(table.stageReferenceName)).length}/{table.count}
			  </p>
			</div>
