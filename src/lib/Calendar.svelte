...
<div class="flex flex-col overflow-hidden">
	<div class="p-2 bg-background-100 grid rounded-md">
		...
		{#if $view == "d"}
			...
			<div class="grid item" in:zoom={{ delay: 200, duration: 200, scale: 0.8 }} out:zoom={{ duration: 200, scale: 0.8 }}>
				{#key $month}
					<div
					...
					>
						...
						<!--
							<button
								...
								disabled={(blockPastDays && new Date().getTime() + 1000 * 60 * 60 * 24 * 6 - day.JSDate.getTime() > 0) ||
			...
			>
				{#each Array(12) as _, i}
					<button
						...
						class="
                col-start-{(i + 1) % 4}
				{i + 1 == $month ? 'text-accent-500 bg-secondary-200 hover:bg-secondary-300' : 'text-text-600 hover:bg-background-200'}
