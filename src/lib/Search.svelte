...
<form method="get" class="w-full flex flex-row flex-nowrap relative group">
	...
	<div class="w-full flex flex-row flex-nowrap peer">
		...
		/>
		{#if value.trim() != ""}
			<div class="w-fit flex flex-row gap-1 bg-background-100 peer-focus:brightness-95 rounded-r-lg p-2">
				<button on:click={resetValue} type="reset" class="bg-primary-500 hover:bg-primary-600 rounded-md text-text-50 p-1 px-4"
					>Restart</button
				>
			</div>
		{/if}
