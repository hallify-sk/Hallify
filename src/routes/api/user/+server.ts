...
import { createSession, generateSessionToken, hashPassword, setSessionTokenCookie } from '$lib/server/auth.js'; 
...
export async function POST({ request, cookies, locals, getClientAddress }) {
	...
	const user = await User.create({email, password_hash: passwordHash, permission_id: 1, first_name: name, last_name: surname});
