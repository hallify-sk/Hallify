...
<div class="flex flex-row flex-nowrap justify-stretch h-screen pl-64">
	<div class="flex flex-col flex-nowrap h-full w-full">
		<div class="w-full px-2 gap-8 flex justify-stretch pb-6 h-full">
			<div
				class="bg-background-50 mt-24 py-6 col-span-12 rounded-md self-stretch w-full px-10 flex flex-col max-h-full"
			>
				...
				<div bind:this={tableWrapper} class="overflow-auto w-full">
					<table
						class="w-full mt-8 rounded-md overflow-hidden leading-[3rem] text-text-500 border-separate"
						cellspacing="0"
					>
						<colgroup>
							<col class="w-10" />
...
<Popup bind:openPopup={openConfirmModal} bind:closePopup={closeConfirmModal}>
	<div class="w-80">
		...
		<p class="text-text-500 mb-4">
			Na vymazanie sál{checkboxes.filter((t) => t?.checked === true).length > 1 ? '' : 'y'} potrebujete
			zadať dôvod, ktorý bude poslaný uživateľom cez E-Mail. Každá rezervácia v tejto sále sa ihneď vymaže.
		...
		>
			<textarea
				placeholder="Dôvod na zrušenie sál{checkboxes.filter((t) => t?.checked === true).length > 1
					? ''
					: 'y'}"
				maxlength="600"
				minlength="30"
