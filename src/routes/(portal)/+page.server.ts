...
export const actions = {
	...
	addToDateTemp: async ({ request, locals }) => {
		...
		try {
			...
			toUpdate = await (locals.pb as PocketBase).collection("temp_reservations").getFirstListItem(`user="${locals.user.id}"`);
		} catch (e) {
			try {
				toUpdate = await (locals.pb as PocketBase).collection("temp_reservations").create({ user: locals.user.id, date, expires });
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
			} catch (e: any) {
				if (e?.data?.data?.date) {
					return fail(401, {
		...
		try {
			...
			await (locals.pb as PocketBase).collection("temp_reservations").update(toUpdate.id, formData);
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
		} catch (e: any) {
			return fail(500, {
				incorrect: true,
...
export async function load({ locals }) {
	return {
		tempReservations: await (locals.pb as PocketBase).collection("temp_reservations").getFullList({ fields: "id,expires,date,user" }),
		...
		reservations: await (locals.pb as PocketBase).collection("reservations").getFullList({ sort: "created", fields: "id,expires,date,user" }),
